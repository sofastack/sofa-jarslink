...
import com.alipay.titan.api.ModuleRuntimeException;
    ...
    private <T> Map<String, T> scanActions(ApplicationContext applicationContext, Class<T> type,
        ...
        for (T action : applicationContext.getBeansOfType(type).values()) {
            ...
            if (isBlank(actionName)) {
                throw new ModuleRuntimeException("Titan scanActions actionName is null");
            }
            ...
            if (LOGGER.isInfoEnabled()) {
                LOGGER.info("Titan Scan action: {}: bean: {}", key, action);
            }
            actions.put(key, action);
        }
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info("Titan Scan actions finish: {}", ToStringBuilder.reflectionToString(actions));
        }
